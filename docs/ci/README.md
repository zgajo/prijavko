...
| [`spec_drift.yml`](../../.github/workflows/spec_drift.yml) | Every `push` and `pull_request` | Runs [`tools/spec_drift_check.sh`](../../tools/spec_drift_check.sh): asserts `minSdk`/`targetSdk`/`compileSdk` in Gradle match PRD/README, required story artifacts (design tokens, cert pins, network security config, env switch) exist, and `AndroidManifest.xml` keeps `allowBackup=false` + `fullBackupContent=false` (NFR-S6). | Merge blocked. A spec ↔ code mismatch surfaces with the specific check that failed and the source-of-truth doc. |
